...
export const getDocument = async ({
  ...
  try {
    ...
    if (!hasAcces) throw new Error("Ypu do not hace acces to this document");

    return parseStringify(room);
  } catch (error) {
    console.log(`Error happend while getting a rom : ${error}`);
  }
};
...
export const updateDocument = async (roomId: string, title: string) => {
  try {
    ...
    console.log(`Error happend whileupdating a room ${error}`);
  }
};
